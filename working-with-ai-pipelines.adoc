...
permalink: /docs/working-with-ai-pipelines
...
:toc: left
:compat-mode:
:context: ai-pipelines
 
[role='_abstract']
As a data scientist, you can enhance your projects in {productname-short} by building portable machine learning (ML) workflows with AI pipelines, using Docker containers. This enables you to standardize and automate machine learning workflows to enable you to develop and deploy your data science models.
...
ifndef::upstream[]
You can also use the Elyra JupyterLab extension to create and run AI pipelines within JupyterLab. For more information, see link:{rhoaidocshome}{default-format-url}/working_with_ai_pipelines/working-with-pipelines-in-jupyterlab_ai-pipelines[Working with pipelines in JupyterLab].
endif::[]
ifdef::upstream[]
You can also use the Elyra JupyterLab extension to create and run AI pipelines within JupyterLab. For more information, see link:{odhdocshome}/working-with-ai-pipelines/#working-with-pipelines-in-jupyterlab_ai-pipelines[Working with pipelines in JupyterLab].
endif::[]

To use a pipeline in {productname-short}, you need the following components:
...
This feature is based on Kubeflow Pipelines 2.0. Use the latest Kubeflow Pipelines 2.0 SDK to build your AI pipeline in Python code. After you have built your pipeline, use the SDK to compile it into an Intermediate Representation (IR) YAML file. The {productname-short} user interface enables you to track and manage pipelines, experiments, and pipeline runs. To view a record of previously executed, scheduled, and archived runs, you can go to *Develop & train* -> *Pipelines* -> *Runs*, or you can select an experiment from the *Experiments* -> *Experiments and runs* to access all of its pipeline runs.
...
include::assemblies/managing-ai-pipelines.adoc[leveloffset=+1]
