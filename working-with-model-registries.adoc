...
As an OpenShift cluster administrator, if the model registry was not enabled in a previous {productname-short} version, after upgrading, you must enable this component for {productname-short} administrators and data scientists to use. 
...
As a data scientist in {productname-short}, you can store, share, version, deploy, and track models by using the model registry. 

include::assemblies/working-with-model-registries.adoc[leveloffset=+1]

= Working with the model catalog

As a data scientist in {productname-short}, you can discover and evaluate the generative AI models that are available in the model catalog. From the model catalog, you can select models that you want to register, deploy, and customize.
