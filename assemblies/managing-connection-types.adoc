...
When a data scientist creates a connection, they start by selecting a connection type. {productname-short} includes pre-installed connection types for S3-compatible object storage databases and URI-based repositories. Connection types include customizable fields and optional default values to decrease the time required by a user to add connections to data sources and sinks. 
