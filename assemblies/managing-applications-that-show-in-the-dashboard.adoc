:_module-type: ASSEMBLY

ifdef::context[:parent-context: {context}]

:context: dashboard

[id="managing-applications-that-show-in-the-dashboard"]
= Managing applications that show in the dashboard
:upstream:
//:preview:

ifdef::preview[]
include::../_artifacts/document-attributes-global.adoc[]
endif::preview[]


include::modules/adding-an-application-to-the-dashboard.adoc[leveloffset=+1]
include::modules/preventing-users-from-adding-applications-to-the-dashboard.adoc[leveloffset=+1]
include::modules/disabling-applications-connected.adoc[leveloffset=+1]
include::modules/showing-hiding-information-about-available-applications.adoc[leveloffset=+1]
include::modules/hiding-the-default-basic-workbench-application.adoc[leveloffset=+1]


ifdef::parent-context[:context: {parent-context}]
ifndef::parent-context[:!context:]
