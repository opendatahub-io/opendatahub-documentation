...
* *Regex Detectors*: Pattern-based content detection for structured rule enforcement. These are the build-in detectors in the Guardrails Orcehstrator service. Learn more about the link:https://github.com/trustyai-explainability/guardrails-regex-detector[guardrails-regex-detector].

* *Hugging Face Detectors*: Content detection compatible with most Hugging Face `AutoModelForSequenceClassification` models, such as `granite-guardian-hap-38m` or `deberta-v3-base-prompt-injection-v2`. Learn more about the detector algorithms for the link:https://github.com/trustyai-explainability/guardrails-detectors[FMS Guardrails Orchestrator].
...
The following sections show how to deploy Guardrails Orchestrator and outline a number of use cases:

* Deploy a Guardrails Orchestrator instance
* Monitor user-inputs to your LLM
* Configure and use the regex detector
* Configure and use the guardrails gateway
* Enable the OpenTelemetry exporter for observing metrics and tracing
* Use Hugging Face models as detectors in the Guardrails Orchestrator service
* Use Guardrails Orchestrator with Llama-stack
