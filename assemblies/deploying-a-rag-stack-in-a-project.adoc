...
include::modules/overview-of-qdrant-vector-databases.adoc[leveloffset=+2]
...
[NOTE]
====
This assembly uses Milvus as the example remote vector database deployment. For other supported vector database options, see the provider-specific documentation:

* xref:using-qdrant-in-llama-stack_{context}[Using Qdrant in Llama Stack]
* xref:using-postgresql-in-llama-stack_{context}[Using PostgreSQL in Llama Stack]
====
