...
ifndef::upstream[]
* To learn how to use the built-in detectors with `trustyai_fms` Orchestrator server external provider for Llama Stack to detect PII, see xref:detecting-pii-by-using-guardrails-with-llama-stack.adoc#detecting-pii-by-using-guardrails-with-llama-stack_safety[Detecting personally identifiable information (PII) by using Guardrails with Llama Stack].
endif::[]
