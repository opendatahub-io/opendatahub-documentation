...
* `type` - The kind of detector server. For now, the only supported kind within {productname-short} is `text_contents`
...
A TLS configuration consists of the following fields:
    ...
    * `ca_path` - The path to CA certificate `.crt` file on the Guardrails Orchestrator container. The default {openshift-platform} Serving CA is mounted at `/etc/tls/ca/service-ca.crt`, which can be used as your `ca_path`.
+
...
the various services specified in this configuration. If you want to ensure that the Orchestrator can talk to authenticated services, specify `"authorization"` and `"content-type"`.
|===