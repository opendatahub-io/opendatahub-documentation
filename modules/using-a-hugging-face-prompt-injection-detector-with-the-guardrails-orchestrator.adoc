...
* You have a large language model (LLM) for chat generation or text classification, or both, deployed in your namespace, in order to follow the Orchestrator API example.
...
You can also use these types of detectors to perform standalone detections using a Detector API (as well as performing standaolone detections with the Orchestrator API).
