...
[NOTE]
--
These examples consider text that may contain offensive language, as they demonstrate usage of detectors that are designed to filter out offensive and/or malicious content. 
--
.Prerequisites
...
. Apply `detector_model_storage.yaml` to download the required detector model(s) from link:https://huggingface.co/models[Hugging Face Model Hub] and place it in a storage location:
+
[source,bash]
----
...
* The HAP detector can be configured in a similar way to the Prompt Injection detector. See link:https://github.com/trustyai-explainability/trustyai-llm-demo/tree/main/guardrails/hap_detector[this demo] on how to configure serving runtime and inference services for this detector.

* To see an example on how to deploy a text generation LLM (named `qwen2` in the example, as opposed to `llm` like in the `ConfigMap` below), see link:https://github.com/trustyai-explainability/trustyai-llm-demo/tree/main/vllm[this demo]. 
--
