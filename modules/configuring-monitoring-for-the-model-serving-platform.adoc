...
ifdef::upstream[]
The model serving platform includes metrics for link:{odhdocshome}/configuring-your-model-serving-platform/#supported-model-serving-runtimes_odh-admin[supported runtimes] of the KServe component. KServe does not generate its own metrics, and relies on the underlying model-serving runtimes to provide them. The set of available metrics for a deployed model depends on its model-serving runtime.
