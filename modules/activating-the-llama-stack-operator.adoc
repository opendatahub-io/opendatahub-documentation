...
. Confirm that one or more pods for the Llama Stack Operator appear in the *redhat-ods-applications* namespace and reach the *Running* state. The `managementState` field in the `DataScienceCluster` CR will also reflect `Managed`.
+
[NOTE]
====
