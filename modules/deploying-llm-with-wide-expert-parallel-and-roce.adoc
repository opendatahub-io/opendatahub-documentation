...
* Your cluster infrastructure can support deploying high scale inference:
** You have high speed inter-accelerator networking.
** The pods using inter-node EP are deployed in a cluster environment with full mesh network connectivity.
+
[NOTE:] The DeepEP backend used in WideEP requires All-to-All RDMA connectivity. Every NIC on a host must be able to communicate with every NIC on all other hosts. Networks restricted to communicating only between matching NIC IDs, for example, rail-only connectivity, will fail.
+
** You have deployed the optional `LeaderWorkerSet` controller.
...
* You have enough GPUs to distribute expert layers: minimum 4 GPUs recommended for Mixtral-8x7B.
