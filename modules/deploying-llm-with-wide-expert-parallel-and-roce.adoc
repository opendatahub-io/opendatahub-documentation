...
----
+
. *expert_parallelism.num_experts_per_gpu*: Number of expert layers to place on each GPU. For Mixtral-8x7B with 4 GPUs, use 2.
. *expert_parallelism.routing_strategy*: `load_balanced`, `random`, or `token_choice`.
. *nccl.ib_hca*: InfiniBand/RoCE HCA (Host Channel Adapter) prefix.
...
----
+
. *replicas*: Equals the expert parallelism degree.
. *parallelism.expertParallel*: Distributes 8 experts across 4 GPUs, 2 experts per GPU.
. *VLLM_EXPERT_PARALLEL_SIZE*: Must match the `expertParallel` value.
. *shm volume*: For inter-process communication.
