...
* You have a Mixture-of-Experts model (for example, Mixtral-8x7B, Mixtral-8x22B).
...
data:
  config.yaml: |
    expert_parallelism:
      enabled: true
      num_experts_per_gpu: 2  # <1>
      routing_strategy: "load_balanced"  # <2>
      communication:
        backend: nccl
        use_rdma: true
    nccl:
...
----
<1> Number of expert layers to place on each GPU. For Mixtral-8x7B with 4 GPUs, use 2.
<2> Expert routing strategy: `load_balanced`, `random`, or `token_choice`.
<3> InfiniBand/RoCE HCA (Host Channel Adapter) prefix.
      ...
      spec:
        ...
        - name: config
          configMap:
            name: wideep-config
----
<1> Number of replicas equals the expert parallelism degree.
<2> Expert parallelism degree - distributes 8 experts across 4 GPUs (2 experts per GPU).
<3> Must match the `expertParallel` value.
...
* *OOM errors*: Reduce `num_experts_per_gpu` or increase GPU memory. Consider using quantization (INT8/FP8).
