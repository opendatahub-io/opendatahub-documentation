...
. In the editor, enter the YAML code to apply the annotation `opendatahub.io/recommended-accelerators`. The excerpt in this example shows the annotation to set a recommended tag for NVIDIA GPU accelerated hardware:
+
[source,yaml]
metadata:
	annotations:
