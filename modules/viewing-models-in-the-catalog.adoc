...
[role='_abstract']
You can discover and evaluate the available generative AI models in the model catalog to find the best fit for your use cases. You can select from the available model categories, search by text, and filter by labels. 
...
. For models in the *Red Hat AI validated models* category only, click the *Performance Insights* tab to view performance benchmark data to compare performance metrics for specific hardware configurations and determine the most suitable option for deployment.  
+
...
** `TTFT` (time to first token): The time that the user must wait before seeing the model output. 
** `TPS` (tokens per second): The total number of tokens output per second.
** `ITL` (inter-token latency): The the average time taken between consecutive tokens.
.. Select a percentile value in milliseconds from the list: `Mean`, `P90`, `P95`, or `P90`.
.. Use the slider to set the maximum acceptable latency value in milliseconds.
.. Click *Apply filter*.
* *Min RPS*: Only show models that can handle at least this number of requests. Hardware configurations that perform below this value are hidden.
