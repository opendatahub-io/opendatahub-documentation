...
[IMPORTANT]
==== 
Upgrading from {productname-short} version 2.x to 3.x changes the dashboard URL format. The cluster administrator must provide the new dashboard URL to all {productname-long} administrators and users as the previous dashboard URL format is no longer functional. 
====
