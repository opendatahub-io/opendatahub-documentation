...
= Enforcing the local-queue labelling policy in all projects

[role='_abstract']
The local-queue labelling policy is enforced in all projects by default.

If the original `ValidatingAdmissionPolicyBinding` resource was edited, you can follow these steps to restore the default values.
...
+
.Example validating admission policy binding for all projects
