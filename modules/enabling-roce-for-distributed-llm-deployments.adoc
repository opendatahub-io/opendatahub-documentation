...
gdrcopy.enabled:: Specifies whether to enable GDRCopy, which provides additional performance optimizations for GPU-to-GPU transfers and is required for low-latency memory copying. Set to `true` if your environment supports it. 
+
...
device:: Specifies the host network device name. Replace `enp163s0` with your actual device name.
...
+
To attach the network interfaces to your pods, add annotations to the pod specification:
+
[source,yaml]
----
metadata:
