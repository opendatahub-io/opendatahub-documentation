...
----
+
. *gdrcopy.enabled*: GDRCopy provides additional performance optimizations for GPU-to-GPU transfers. Set to `true` if your environment supports it.
+
...
----
+
. *nicSelector.vendor*: Mellanox/NVIDIA vendor ID. Adjust for your network card vendor.
. *nicSelector.deviceID*: Device ID for your specific network card model.
. *isRdma*: Enable RDMA support for RoCE.
...
----
+
. *k8s.v1.cni.cncf.io/networks*: Attach the RoCE secondary network to the pod.
. *rdma/roce*: Request RDMA resources. The resource name depends on your SR-IOV or network configuration.
. *NCCL_IB_DISABLE*: Enable InfiniBand/RoCE for NCCL (NVIDIA Collective Communications Library).
. *NCCL_NET_GDR_LEVEL*: Set GPUDirect RDMA level: 0-5, where 5 is maximum optimization.
. *NCCL_DEBUG*: Enable debug logging for troubleshooting.
