...
* If you are running {openshift-platform} on a bare metal cluster:  Your cluster administrator has provisioned an external entry point for the `openshift-ai-inference` Inference Gateway service with the type `LoadBalancer`. Ensure that the `LoadBalancer` is configured as follows:
