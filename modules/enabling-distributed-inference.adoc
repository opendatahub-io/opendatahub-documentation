...
* If you are running {openshift-platform} on a bare-metal cluster, your cluster administrator has an external entry point for the `openshift-ai-inference` Inference Gateway service.
+
[NOTE]
====
By default, the Inference Gateway uses type: `LoadBalancer`. If the cluster does not already include support for `LoadBalancer` services, you can use the OpenShift option described in link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/ingress_and_load_balancing/load-balancing-with-metallb[Load balancing with MetalLB].
====
