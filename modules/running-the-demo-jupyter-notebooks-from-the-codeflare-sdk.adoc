...
* Your workbench image has been updated to the latest version.
//+
//[NOTE]
//====
//After the image upgrade is complete, and before any new RayClusters are created, you must perform manual steps to migrate existing notebooks to function with the new authentication pattern. 
//====
ifdef::upstream[]
* You can access a project that contains a workbench, and the workbench is running a default workbench image that contains the CodeFlare SDK, for example, the *Standard Data Science* workbench. 
...
ifndef::upstream[]
For information about the latest available training images and their preinstalled packages, including the CUDA and ROCm versions, see link:https://access.redhat.com/articles/rhoai-supported-configs-3.x[Supported Configurations for 3.x].
endif::[]
--
...
[NOTE]
====
Mutual Transport Layer Security (mTLS) is enabled by default in the Ray component in {productname-short}.
