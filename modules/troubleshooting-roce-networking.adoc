...
*Symptom*: GPU Operator pods remain in `Pending` or `CrashLoopBackOff` state.
...
*Symptom*: RDMA devices are not available in pods, or `ls /dev/infiniband/` returns empty.
...
*Symptom*: Pods fail to start with error: `error adding container to network "roce-network": network not found`.
...
*Symptom*: SR-IOV Virtual Functions (VFs) are not created on nodes.
...
=== NCCL falls back to TCP instead of using RDMA

*Symptom*: Logs show `NCCL INFO Using network Socket` instead of `NCCL INFO Using network RoCE` or `IB`.
...
=== High RDMA transfer latency

*Symptom*: RDMA transfers are slower than expected, impacting model inference performance.
...
*Symptom*: Pods fail to start with error: `NCCL WARN Call to ibv_create_qp failed` or `NCCL initialization timeout`.
...
*Symptom*: Pods are killed with `OOMKilled` status when loading large models.
...
. Use model quantization (FP8, INT8) to reduce memory footprint:
+
[source,yaml]
----
env:
...
*Symptom*: LLMInferenceService pods remain in `Pending` state.
...
=== Unable to run RDMA bandwidth tests

*Symptom*: `ib_write_bw` or similar tools fail with permission errors or device not found.
...
. Run tests with proper permissions (may require privileged pods for some operations):
+
[source,yaml]
----
securityContext:
...
=== Cluster network not available

*Symptom*: Secondary network interfaces are not created on IBM Cloud.
