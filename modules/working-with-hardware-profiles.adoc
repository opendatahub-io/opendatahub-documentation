...
In {productname-long}, you can manage and allocate specific hardware resources, such as hardware accelerators, specialized memory, or CPU-only nodes for data science workloads using *hardware profiles*. 

A hardware profile is a custom resource definition (CRD) that you create in {productname-short} to specify a particular hardware configuration. You can create and manage these profiles by going to *Settings* -> *Hardware profiles* on the {productname-short} dashboard.

You can find hardware profiles and the `OdhDashboardConfig` 
