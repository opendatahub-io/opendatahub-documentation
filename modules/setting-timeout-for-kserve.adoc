...
[NOTE]
====
Ensure that you set the `progress-deadline` on the `spec.predictor.annotations` level, so that the InferenceService can copy the `progress-deadline` back to the KNative Service object.
====
