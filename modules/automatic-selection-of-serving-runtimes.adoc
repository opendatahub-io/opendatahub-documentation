...
The system suggests a runtime by matching the accelerated hardware defined in your selected hardware profile with available runtimes. For example, if you select a hardware profile that uses NVIDIA GPU accelerated hardware, the system filters for compatible runtimes, such as *vLLM NVIDIA GPU ServingRuntime for KServe*.
