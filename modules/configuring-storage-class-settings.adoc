...
. To enable access modes in storage class settings, select from the available *Access mode enablement* options.
* ReadWriteOnce (RWO): Enabled by default. The storage can be attached to a single workbench at a given time.
