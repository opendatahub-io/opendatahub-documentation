...
[id="configuring-authz-policies-for-llmd_{context}"]
...
spec:
  targetRef:
    ...
    name: <llm-inference-service-name> <1>
  rules:
    authentication:
        ...
        kubernetesSubjectAccessReview:
          ...
          resourceAttributes:
            ...
            verb:
              value: get
----
<1> Replace with the name of your `LLMInferenceService`.
<2> This validates that the ServiceAccount has permission to get the specific `LLMInferenceService` resource.
      ...
      "allowed-service-accounts":
        patternMatching:
          ...
          - selector: auth.identity.sub
            operator: matches
            value: ^system:serviceaccount:<namespace>:(llm-user-1|llm-user-2)$ <1>
----
<1> Replace with a regular expression that matches the allowed ServiceAccount names.
