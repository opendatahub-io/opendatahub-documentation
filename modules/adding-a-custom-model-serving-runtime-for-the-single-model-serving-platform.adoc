...
ifdef::upstream[]
A model-serving runtime adds support for a specified set of model frameworks (that is, formats). You have the option of using the link:{rhodsdocshome}{default-format-url}/serving_models/serving-large-language-models_serving-large-language-models#about-the-single-model-serving-platform_serving-large-language-models[pre-installed runtimes] included with {productname-short} or adding your own, custom runtimes. This is useful in instances where the pre-installed runtimes don't meet your needs. For example, you might find that the TGIS runtime does not support a particular model format that is supported by link:https://huggingface.co/docs/text-generation-inference/supported_models[Hugging Face Text Generation Inference (TGI)^]. In this case, you can create a custom runtime to add support for the model.
endif::[]

ifdef::self-managed,cloud-service[]
A model-serving runtime adds support for a specified set of model frameworks (that is, formats). You have the option of using the link:{rhoaidocshome}{default-format-url}/serving_models/serving-large-language-models_serving-large-language-models#about-the-single-model-serving-platform_serving-large-language-models[pre-installed runtimes] included with {productname-short} or adding your own, custom runtimes. This is useful in instances where the pre-installed runtimes don't meet your needs. For example, you might find that the TGIS runtime does not support a particular model format that is supported by link:https://huggingface.co/docs/text-generation-inference/supported_models[Hugging Face Text Generation Inference (TGI)^]. In this case, you can create a custom runtime to add support for the model.
endif::[]
