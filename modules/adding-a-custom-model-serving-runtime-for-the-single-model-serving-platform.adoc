...
* You have built you custom runtime and added the image to a container image repository such as link:quay.io[Quay^].
...
--
+
Your YAML code might be a custom runtime that you've created to support a link:https://github.com/huggingface/text-generation-inference/tree/main[Hugging Face Text Generation Inference (TGI)^] model, for example. In some cases, creating a custom runtime might involve adding new or custom parameters to the `env` section of the `ServingRuntime` specification.
