...
[NOTE]
====
{productname-short} model serving deployments use the global cluster pull secret to pull models in OCI-compliant ModelCar format from the catalog. 
...
+
ifndef::upstream[]
For more information about how the system determines the best runtime and administrator overrides, see link:{rhoaidocshome}{default-format-url}/deploying_models/deploying_models#automatic-selection-of-serving-runtimes_rhoai-user[Automatic selection of serving runtimes].
endif::[]
ifdef::upstream[]
For more information about how the system determines the best runtime and administrator overrides, see link:{odhdocshome}/deploying_models/#automatic-selection-of-serving-runtimes_odh-user[Automatic selection of serving runtimes].
endif::[]
...
*** *Rolling update*: Existing inference service pods are terminated _after_ new pods are started. This ensures zero downtime and continuous availability. This is the default option. 
*** *Recreate*: Existing inference service pods are terminated _before_ new pods are started. This saves resources but guarantees a period of downtime.
