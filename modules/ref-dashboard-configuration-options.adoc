...
| `spec.dashboardConfig.` +
`disableFineTuning` | No longer used | *DEPRECATED* Hides the *Models → Model customization* menu item in the dashboard navigation menu and the button for registered model versions. 
...
| `spec.dashboardConfig.` +
`disableLLMd` | `false` | Shows the *Enable distributed inference with llm-d* switch on the *Settings → General settings* page. To hide this switch, set the value to `true`. 
| `spec.dashboardConfig.` +
