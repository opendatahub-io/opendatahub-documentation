...
(Technology Preview) | `true` | Enables new projects to automatically receive a local queue and allows administrators to use hardware profiles configured with a default queue name and priority in their workloads, rather than relying on node selectors and tolerations.

Kueue is a Technology Preview feature in this {productname-short} release.
| `spec.dashboardConfig.` +
`disableLMEval` 

(Technology Preview) | `true` | Shows the *Model* > *Model evaluation runs* menu item in the dashboard navigation menu. 
