...
| `spec.dashboardConfig.` +
`disableKueue` | `true` | Hides Kueue-related options in the dashboard. Set the value to `false` to enable Kueue in the dashboard, which allows users to select Kueue-enabled hardware profiles and automatically configures new projects for Kueue management by applying the required label and creating a default local queue.
