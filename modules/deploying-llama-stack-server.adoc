...
* You have deployed and prepared a PostgreSQL database for Llama Stack metadata storage. 
* You have a running PostgreSQL instance that is reachable from the {productname-short} cluster network.
...
----
+
<1> Create the secret in the same namespace as the `LlamaStackDistribution` resource. Avoid placing passwords directly on the command line, as they can be stored in shell history. Instead, create the secret from a file or by using the OpenShift web console. For example, run `oc create secret generic postgres-secret --from-file=password=<password filename> -n <namespace>`.
+
Delete the temporary file after creating the secret to prevent accidental exposure of credentials. For more information about creating and managing Secrets, see the OpenShift documentation.
...
. Start the PostgreSQL shell. For example:  
+
[source,terminal]
----
$ psql -h <host> -p 5432 -U <admin-user> -d postgres
----
