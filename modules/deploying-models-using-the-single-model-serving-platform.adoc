...
[id="deploying-models-using-the-model-serving-platform_{context}"]
= Deploying models by using the model serving platform

[role='_abstract']
On the model serving platform, each model is deployed on its own model server. This helps you to deploy, monitor, scale, and maintain large models that require increased resources.

ifdef::upstream[]
[IMPORTANT]
====
If you want to use the model serving platform to deploy a model from S3-compatible storage that uses a self-signed SSL certificate, you must install a certificate authority (CA) bundle on your OpenShift cluster. For more information, see link:{odhdocshome}/installing-open-data-hub/#understanding-certificates_certs[Understanding how {productname-short} handles certificates].
====
endif::[]
