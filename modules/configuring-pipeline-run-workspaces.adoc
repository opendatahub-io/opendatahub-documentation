...
A workspace is a standardized directory structure backed by a Persistent Volume Claim (PVC) that gets dynamically created for each pipeline run based on the DSPA workspace configuration. When the pipeline definition includes a workspace configuration, the DSPA automatically mounts the workspace PVC into each pipeline task pod at a path that is resolved at runtime by using the dsl.WORKSPACE_PATH_PLACEHOLDER variable as an input parameter to components.

Pipelines opt in to workspace support at compile time. When the pipeline definition includes a workspace configuration, the pipeline compiler annotates the run so that the DSPA automatically creates the workspace PVC and mounts the volume into each task pod.

== Configuring default workspace PVC settings in DSPA

You can configure the default values for workspace PersistentVolumeClaims (PVCs) in your DataSciencePipelinesApplication (DSPA). This allows you to specify the storage class and access mode for workspace volumes used by pipeline runs. Workspace storage size must be provided when you define the pipeline run.
...
== Adding OCI artifacts to pipeline run workspaces

When working with model artifacts stored in OCI (Open Container Initiative) registries, such as Modelcar artifacts with `oci://` URIs, you cannot directly reference them in your pipeline tasks. Instead, you must copy these artifacts into the workspace volume where your tasks can access them.

[NOTE]
====
Until the `dsl.importer` `download_to_workspace` option is available in {productname-short}, use this copy-to-workspace approach for all external artifacts. We will revise the documentation once native workspace downloads are supported so it applies only to OCI artifacts.
====

.Prerequisites
* You have configured a pipeline server in your data science project with workspace support enabled.
* You have access to an OCI registry containing your Modelcar artifacts.
* Your pipeline server has network access to the OCI registry.

.Procedure
To use OCI artifacts in your pipeline, define a component that copies the artifact to the workspace, and then call this component in your pipeline:

. Define a component that copies the OCI artifact to the workspace:
+
[source,python]
----
from kfp import dsl
...
def modelcar_to_workspace_pipeline(
    oci_uri: str = "oci://service-catalog-models/stable-diffusion-xl-base-1.0:1.0",
):
    # Import the model from OCI registry
...
* link:https://www.kubeflow.org/docs/components/pipelines/user-guides/core-functions/persistent-volume/[Kubeflow Pipelines - Using Persistent Volumes]
