...
When working with external artifacts such as Modelcar images stored in an OCI (Open Container Initiative) registry, you cannot directly reference them in your pipeline tasks. Instead, copy these artifacts into the workspace volume so that task pods can access them during execution.
