...
ifndef::upstream[]
To use model registries in {productname-short}, an OpenShift cluster administrator must configure the model registry component in OpenShift. For more information, see llink:{rhoaidocshome}{default-format-url}/configuring_the_model_registry_component/configuring-the-model-registry-component_model-registry-config[Configuring the model registry component].
