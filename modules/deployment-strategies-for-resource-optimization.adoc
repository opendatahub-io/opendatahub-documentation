...
+
* Trade-off: This method requires increased resources (CPU, memory, and GPUs) during the update process. Plan for temporary headroom (approximately 200% of the pod requests) during the transition because parallel instances exist briefly.
...
+
* Trade-off: 
This method guarantees a period of downtime. The model endpoint is unavailable and returns errors (typically 503 Service Unavailable) between the termination of the old pod and the readiness of the new pod.
