...
The {openshift-platorm} MySQL template uses the `caching_sha2_password` authentication plugin by default. If you used the {openshift-platform} template for MySQL, you must manually update it to use `mysql_native_password` in order for the model registry to connect to your database instance successfully. 
ifndef::upstream[]
