...
<2> Specifies voluntary disruption tolerance for the pods. For example, in a voluntary distribution, this configuration keeps at least one server Pod available.
...
<5> Configures and uses the node’s zone label as the failure-domain for Pod spreading.
...
----
+
<1> Configures HorizontalPodAutoscaler for the server pods.
