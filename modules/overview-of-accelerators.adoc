...
** AMD GPUs can accelerate a variety of workloads, including AI/ML training and inference tasks.
** To enable AMD GPUs on OpenShift, you must install the AMD GPU Operator. The AMD GPU Operator manages driver installation, Kubernetes device plugin setup, and node labeling for GPU scheduling.
** Before you enable AMD GPUs, ensure that your OpenShift cluster meets the hardware and software compatibility requirements. These requirements include supported OpenShift versions and AMD GPU models.
** For new deployments, you must also configure an internal image registry and add the default (inbox) AMD GPU drivers to a denylist to avoid conflicts.
** AMD GPU support includes integration with ROCm, the AMD open software platform for GPU compute. 
