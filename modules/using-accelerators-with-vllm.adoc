...
ifdef::upstream[]
To skip warm-up, follow the steps described in link:{odhdocshome}/serving-models/#deploying-models-using-the-single-model-serving-platform_serving-large-models[Customizing the parameters of a deployed model-serving runtime] in set the following vLLM environment variable in the *Configuration parameters* section of your mo:
[source]
----
`VLLM_SKIP_WARMUP="true"`
----
