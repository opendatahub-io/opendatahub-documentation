...
ifdef::upstream[]
To skip warm-up, follow the steps described in link:{odhdocshome}/configuring-your-model-serving-platform[Customizing the parameters of a deployed model-serving runtime] to add the following environment variable in the *Configuration parameters* section of your model deployment:
[source]
----
`VLLM_SKIP_WARMUP="true"`
----
...
ifndef::upstream[]
* link:{rhoaidocshome}{default-format-url}/configuring_your_model_serving_platforn/rhoai-admin_rhoai-admin#supported-model-serving-runtimes_rhoai-admin[Supported model-serving runtimes^]
endif::[]
ifdef::upstream[]
