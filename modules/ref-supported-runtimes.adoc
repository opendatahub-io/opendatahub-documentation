...
|===
| Name | Protocol | Model mesh support | Single node OpenShift support | Deployment mode

| Caikit Text Generation Inference Server (Caikit-TGIS) Serving Runtime for KServe | REST | No | Yes | Raw and serverless

| Caikit Standalone ServingRuntime for KServe | REST and gRPC | No | Yes | Raw and serverless 

| OpenVINO Model Server | REST | Yes | Yes | Raw and serverless 

| Text Generation Inference Server (TGIS) Standalone Serving Runtime for KServe (3) | gRPC | No | Yes | Raw and serverless

| vLLM | REST | No | Yes | Raw and serverless 
