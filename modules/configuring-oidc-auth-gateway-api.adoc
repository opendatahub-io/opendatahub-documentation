...
** To configure OpenShift for direct authentication using ROSA, follow the appropriate {rosa-productname} documentation: link:https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws/{rosa-latest-version}/html/install_clusters/rosa-hcp-cluster-no-cni#rosa-sts-byo-oidc_rosa-hcp-cluster-no-cni[Creating an OpenID Connect configuration].
