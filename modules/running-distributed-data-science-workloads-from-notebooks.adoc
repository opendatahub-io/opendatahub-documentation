...
+
To download the demo notebooks, run the `copy_demo_nbs()` function in a Jupyter notebook, as follows:
+
.Download the demo notebooks
[source,bash]
----
...
+
The `copy_demo_nbs()` function accepts two arguments:
...
+
----
copy_demo_nbs("my-dir")
copy_demo_nbs("/absolute/path/to/target-dir")
copy_demo_nbs("my-dir", True)
copy_demo_nbs(dir="my-dir", overwrite=True)
----

. Locate the downloaded demo notebooks in the JupyterLab interface.
In the left navigation pane, double-click *demo-notebooks*, and then double-click *guided-demos*. 
