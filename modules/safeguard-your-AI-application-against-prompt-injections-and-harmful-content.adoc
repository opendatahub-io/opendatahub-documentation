...
This tutorial uses three main detector models to monitor the request-response lifecycle. These detectors perform harmful content detection, prompt injection mitigation, and language and business compliance.
...
* You have {productname-long} installed on your cluster and a Data Science Cluster (DSC) is deployed.

* You have cluster administrator privileges for your {productname-short} cluster and have configured your cluster for a GPU deployment.
...
. Deploy the Hateful And Profane (HAP) language detector. This uses IBM's *Granite-Guardian-HAP-38m* model:
+
[source, terminal]
----
