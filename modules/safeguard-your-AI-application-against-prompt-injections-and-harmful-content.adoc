...
This tutorial demonstrates how to safeguard an AI application by integrating three detector models into the request-response lifecycle. These detectors monitor incoming prompts and outgoing completions, to identify and triage harmful content, prompt injections, and inappropriate language.

In this tutorial, you carry out the following steps to create your customer service agent:

* *Set up and deploy models:* Deploy a series of models that provide the desired functions.
...
* You have the {productname-long} operator installed with the TrustyAI component set to `Managed`

* You have cluster administrator privileges and have configured your cluster for a GPU deployment.

* In the {productname-short} operator menu, the `servicemesh` component is set to `Removed` in the Data Science Cluster (DSC), so that the LLM is deployed as a KServe Raw deployment.
...
. Create a new project and set a container to deploy the LLM to be safeguarded:
+
[source,YAML]
----
...
[NOTE]
====
The first time the container starts, it might take a few minutes because it needs to download the model. The model pod, once active, should look something like `phi3-predictor-XXXXXX`. 
====

. In a new terminal window, test the model by sending some inferences to it:
+
[source,terminal]
----
...
. Deploy the Hateful And Profane (HAP) language detector, which spins up a `guardrails-container-deployment-hap-xxxx` pod. 
This uses IBM's *Granite-Guardian-HAP-38m* model:
+
[source, terminal]
----
