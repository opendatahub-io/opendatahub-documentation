...
* **OpenAI SDKs**  
  When you use an OpenAI-compatible SDK (for example, the OpenAI Python client), you must include the `/v1` path suffix in the base URL. For example:
  
  `http://llama-stack-service:8321/v1`

* **Llama Stack SDK (`llama_stack_client`)**  
  When you use the native Llama Stack SDK, set the base URL to the Llama Stack service endpoint without the `/v1` suffix. The SDK automatically appends the correct API paths. For example:
