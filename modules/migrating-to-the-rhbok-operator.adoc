...
To migrate from the embedded Kueue to {rhbok-productname}, you can either perform a clean migration or a direct migration. A clean migration, which is the recommended approach, involves first uninstalling the embedded Kueue component to leave the cluster in a clean state and avoid potential configuration conflicts. A direct migration is faster and changes control from the embedded Kueue to the {rhbok-productname} Operator without first uninstalling it. In this case, the embedded Kueue is removed automatically during the migration. The following procedure includes an optional step for performing a clean migration.
