...
metadata:
  ...
  namespace: {dbd-config-default-namespace} # <3>
  labels: # <4>
...
<3> The `pass:attributes[{dbd-config-default-namespace}]` namespace is the default namespace in which the `ImageStream` CR is created.
...
----
oc describe imagestream my-custom-notebook -n {dbd-config-default-namespace}
----
+
