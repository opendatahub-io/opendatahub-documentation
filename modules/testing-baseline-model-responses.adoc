...
If you need to evaluate multiple models side-by-side with identical prompts, you can enable multi-model comparison mode in the playground.
Multi-model comparison helps you select the best-performing model for your use case by displaying responses from up to three models simultaneously.

For more information, see xref:comparing-model-outputs-in-playground_{context}[Comparing model outputs in the playground].

.Verification

* The model provides responses based on its general knowledge or pre-trained data.
