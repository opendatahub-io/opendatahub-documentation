...
----
You can use the chat templates that are available as `.jinja` files link:https://github.com/opendatahub-io/vllm/tree/main/examples[here] or with the vLLM image under `/opt/app-root/template`. For more information, see link:https://huggingface.co/docs/transformers/main/chat_templating[Chat templates].
====
+
...
ifdef::upstream[]
For detailed configuration guidance, see link:{odhdocshome}/deploying-models/#deploying-models-using-mlserver-runtime_odh-user[Deploying models by using the MLServer runtime].
endif::[]
ifndef::upstream[]
For detailed configuration guidance, see link:{rhoaidocshome}{default-format-url}/deploying_models/deploying_models#deploying-models-using-mlserver-runtime_rhoai-user[Deploying models by using the MLServer runtime].
endif::[]

//// 
//MLServer currently does not support gRPC
--
.gRPC endpoints
* `:443 inference.GRPCInferenceService/ModelInfer`
* `:443 inference.GRPCInferenceService/ModelReady`
