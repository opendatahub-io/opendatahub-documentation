...
ifdef::upstream[]
To skip warm-up, follow the steps described in link:{odhdocshome}/configuring-your-model-serving-platform#adding-a-custom-model-serving-runtime-for-the-model-serving-platform[Customizing the parameters of a deployed model-serving runtime] to add the following environment variable in the *Configuration parameters* section of your model deployment:
[source]
----
`VLLM_SKIP_WARMUP="true"`
----
...
== IBM Spyre AI accelerators on x86 and IBM Z
ifndef::upstream[]
[IMPORTANT]
====
ifdef::self-managed[]
