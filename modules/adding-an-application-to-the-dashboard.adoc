...
ifndef::upstream[]
* The dashboard configuration enablement option is set to `true` (the default). Note that an admin user can disable this ability as described in link:{rhoaidocshome}{default-format-url}/managing_resources/managing-applications-that-show-in-the-dashboard_dashboard#disabling-applications-connected_dashboard[Disabling the ability to add applications to the dashboard].
endif::[]
ifdef::upstream[]
* The dashboard configuration enablement option is set to `true` (the default). Note that an admin user can disable this ability as described in link:{odhdocshome}/managing_resources/managing-applications-that-show-in-the-dashboard_dashboard#disabling-applications-connected_dashboard[Disabling the ability to add applications to the dashboard].
endif::[]
