...
ifndef::upstream[]
* The dashboard configuration enablement option is set to `true` (the default). Note that an admin user can disable this ability as described in link:{rhoaidocshome}{default-format-url}/managing-resources/managing-applications-that-show-in-the-dashboard#preventing-users-from-adding-applications-to-the-dashboard_dashboard[Preventing users from adding applications to the dashboard].
endif::[]
ifdef::upstream[]
