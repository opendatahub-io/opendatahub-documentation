...
ifndef::upstream[]
Based on the example configurations provided in link:{rhoaidocshome}{default-format-url}/enabling_ai_safety/enabling-ai-safety-with-guardrails_safety#guardrails-gateway-config-parameters_safety[Configuring the Guardrails Gateway], the available endpoint for the model with Guardrails is `$GUARDRAILS_GATEWAY/pii`.
endif::[]
