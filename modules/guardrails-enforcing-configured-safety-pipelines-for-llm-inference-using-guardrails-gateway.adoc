...
+
ifdef::upstream[]
Based on the example configurations provided in link:{odhdocshome}/enabling-ai-safety/#guardrails-gateway-config-parameters_safety[Configuring the built-in detector and Guardrails gateway], the available endpoint for the guardrailed model is `$GUARDRAILS_GATEWAY/pii`.
endif::[]
