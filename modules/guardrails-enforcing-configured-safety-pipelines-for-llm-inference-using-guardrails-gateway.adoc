...
+
ifdef::upstream[]
Based on the example configurations provided in link:{odhdocshome}/enabling-ai-safety/#configuring-the-built-in-detector-and-guardrails-gateway_safety[Configuring the built-in detector and Guardrails gateway], the available endpoint for the guardrailed model is `$GUARDRAILS_GATEWAY/pii`.
endif::[]

ifndef::upstream[]
Based on the example configurations provided in xref:guardrails-gateway-config-parameters.adoc#guardrails-gateway-config-parameters_safety[Configuring the Guardrails Gateway], the available endpoint for the model with Guardrails is `$GUARDRAILS_GATEWAY/pii`.
endif::[]
