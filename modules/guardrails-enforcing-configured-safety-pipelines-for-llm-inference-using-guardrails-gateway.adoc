...
+
ifdef::upstream[]
Based on the example configurations provided in link:{odhdocshome}/enabling-ai-safety#guardrails-gateway-config-parameters_safety[Configuring the Guardrails Gateway], the available endpoint for the guardrailed model is `$GUARDRAILS_GATEWAY/pii`.
endif::[]
ifndef::upstream[]
Based on the example configurations provided in link:{rhoaidocshome}{default-format-url}/enabling_ai_safety/enabling-ai-safety_safety#guardrails-gateway-config-parameters_safety[Configuring the Guardrails Gateway], the available endpoint for the guardrailed model is `$GUARDRAILS_GATEWAY/pii`.
endif::[]
