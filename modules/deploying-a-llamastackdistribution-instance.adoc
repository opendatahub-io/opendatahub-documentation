...
* *Example D:* Remote PostgreSQL with pgvector
>>>>>>> 5934319 (RHAI-ENG-2278 - pgvector integration docs draft)
...
* You have installed the PostgreSQL operator and configured a PostgreSQL database, see "Deploying a Llama Stack server".
...
====
<<<<<<< HEAD
To enable the inline embeddings in a disconnected environment, you need add the following parameters to your `LlamaStackDistribution` custom resource. 
=======
To enable Llama Stack in a disconnected environment, add the following parameters to your `LlamaStackDistribution` custom resource. 
>>>>>>> 5934319 (RHAI-ENG-2278 - pgvector integration docs draft)
[source,yaml]
----
...
# Additional required configuration for disconnected
...
The built-in Llama Stack tool `websearch` is not available in the {org-name} Llama Stack Distribution in disconnected environments. In addition, the built-in Llama Stack tool `wolfram_alpha` tool is not available in the {org-name} Llama Stack Distribution in all clusters.
====
...
Use this example when you want to use a PostgreSQL database with the pgvector extension as the vector store backend. This configuration enables the pgvector provider and reads connection values from a secret.

. Create the pgvector connection secret.
+
[source,terminal]
----
export PGVECTOR_HOST="pgvector"
export PGVECTOR_PORT="5432"
export PGVECTOR_DB="vectordb"
export PGVECTOR_USER="postgres"
export PGVECTOR_PASSWORD="<password>"

oc create secret generic pgvector-connection \
...
>>>>>>> aee30b5 (Processing qe and sme feedback)
. Click *Create*.
...
[TIP]
====
If you change the vector store configuration, delete the existing pod to ensure the new environment variables and backing store settings are applied.
====
