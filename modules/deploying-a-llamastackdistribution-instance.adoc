  ...
  server:
    containerSpec:
      ...
      env:
        ...
        # --- Remote Milvus configuration (v2.25: no ConfigMap required) ---
        - name: MILVUS_ENDPOINT
          value: "http://milvus-service:19530" # --- Use the **gRPC** port (`19530`) for `MILVUS_ENDPOINT`. Port `9091` is for health checks.---
        - name: MILVUS_TOKEN
          valueFrom:
            secretKeyRef:
              name: milvus-secret
