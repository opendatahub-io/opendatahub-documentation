...
Prefill/decode disaggregation separates the initial token generation (prefill) from subsequent token generation (decode) into separate GPU pools. RoCE enables high-speed KV cache transfers between the prefill and decode stages, improving throughput and latency.

.Prerequisites

* You have completed the procedure for enabling RoCE networking for distributed LLM deployments.
  ...
  config.yaml: |
    disaggregation:
      ...
      decode:
        ...
        max_batch_size: 256
    kv_cache:
      transfer_method: rdma  # <1>
      rdma_device: mlx5_0  # <2>
    nccl:
      ib_disable: 0
      net_gdr_level: 5
----
<1> Use RDMA for KV cache transfers between prefill and decode stages.
<2> Specify the RDMA device name. Use `ibv_devices` to list available devices.
  ...
  router:
    ...
    scheduler:
      disaggregation:
        enabled: true
----
<1> Prefill pool handles initial token generation with optimized batching.
<2> Decode pool handles subsequent token generation, receiving KV cache from prefill via RDMA.
