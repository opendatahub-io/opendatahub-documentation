...
----
+
. *kv_cache.transfer_method*: Use RDMA for KV cache transfers between prefill and decode stages.
. *kv_cache.rdma_device*: Specify the RDMA device name. Use `ibv_devices` to list available devices.
...
----
+
. *prefillPool*: Handles initial token generation with optimized batching.
. *decodePool*: Handles subsequent token generation, receiving KV cache from prefill via RDMA.
