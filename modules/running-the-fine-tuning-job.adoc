...
. [Optional] Edit the content to specify the appropriate values for your model and dataset.
...
.. Edit the `HF_TOKEN` value to specify your HuggingFace token.
+
If you are not fine-tuning a gated model from HuggingFace Hub, remove the `HF_HOME` and `HF_TOKEN` entries.

.. If your PVC has a different name or mount path, update the PVC details to specify the RWX storage attached to your workbench.
