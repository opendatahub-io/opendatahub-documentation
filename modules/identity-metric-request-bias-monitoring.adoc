...
You can monitor the average values of various data fields over time to see the average ratio of loan-payback to loan-default predictions, as well as the average ratio of male-identifying to non-male-identifying applicants. We can do this by creating an Identity Metric Request by sending a POST request to the `/metrics/identity/request` endpoint:
[source]
----
