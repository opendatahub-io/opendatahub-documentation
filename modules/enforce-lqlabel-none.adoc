...
You can disable the policy for all projects in either of the following ways:
...
The user will be able to create RayClusters and bypass Kueue, unless the local-queue resource has been annotated with ' kueue.x-k8s.io/default-queue: "true" ' and the user's namespace is the same as the default local-queue's namespace. This can be tested by attempting to create a RayCluster without specifying a local-queue in the local_queue field, of which case the creation should be successful.
////