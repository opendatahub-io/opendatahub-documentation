...
. Specify the URI connection annotation in the `metadata.annotations section`. Add the `opendatahub.io/connections` annotation and set its value to reference the URI Secret name.
+
[source,bash]
----
