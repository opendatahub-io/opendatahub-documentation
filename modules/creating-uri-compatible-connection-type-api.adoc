...
In {productname-short}, you can create a URI-compatible connection type by using the connections API. In the following procedure, you define a Kubernetes Secret resource that holds a simple URI for connecting to an external resource, such as a model file hosted on an HTTP server or Hugging Face.
...
----
+
.. In the example YAML, replace the required URI field by populating the placeholder value in  the `stringData` section to include complete URL to the resource. This can be an HTTP/HTTPS link, or a Hugging Face URI. 
+
