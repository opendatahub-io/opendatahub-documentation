...
$ curl -k -H "Authorization: Bearer $KUBEFLOW_PIPELINES_TOKEN" \
 https://$KFP_ENDPOINT/apis/v1beta1/healthz
----
+
You should see a successful health check response confirming that the endpoint is accessible with your token.
...
[IMPORTANT]
====
The KUBEFLOW_LLAMA_STACK_URL must be an external route that is accessible from the Kubeflow Pipeline pods. Internal service URLs do not work for this 
configuration. The Kubeflow Pipeline pods need to communicate with the Llama Stack server from within their execution environment, which requires an externally accessible route.
====
+
The route is automatically created when you deploy the LlamaStackDistribution. You can create the route URL before crating the LlamaStack distribution with the following user-replaced values:
+
[source,subs="+quotes"]
----
http://$<distribution_name>-service.$<your_namespace>.svc.cluster.local:$<port>
----
+
The <distribution_name>, <namespace>, and <port> are replaced with the name of the LlamaStack distribution you are creating, the namespace where you are creating it, and the port. These 3 elements are present in the LlamaStack distribution yaml.
...
data:
  ...
  KUBEFLOW_LLAMA_STACK_URL: "https://<external_llama_stack_url>"
...
* `KUBEFLOW_RESULTS_S3_PREFIX`: the S3 path prefix where evaluation results will be stored.  For example: `s3://my-bucket/ragas-evaluation-results`.
