...
[source,terminal]
----
$ export KUBEFLOW_PIPELINES_ENDPOINT=$(oc get route -n <project_name> -l app=ds-pipeline-dspa -o jsonpath='{.items[0].spec.host}')
----
+
Run the following command to verify that you can see the endpoint:
+
[source,terminal]
----
