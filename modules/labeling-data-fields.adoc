...
* You sent training data to TrustyAI as described in xref:sending-training-data-to-trustyai_monitor[Sending training data to TrustyAI].
...
. In the OpenShift CLI, find the route to the TrustyAI service: 
+
[source]
----
...
+
Change the `model-name`, `original-name`, and `Prediction` values to those used in your model. Change the `New name` values to the labels you want to use. 
+
[source]
----
...
You should see a "Feature and output name mapping successfully applied" message.
