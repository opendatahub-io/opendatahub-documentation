...
ifndef::upstream[]
*Note:* The Jupyter tile is enabled by default. To disable it, see link:{rhoaidocshome}{default-format-url}/managing-resources/managing-applications-that-show-in-the-dashboard#hiding-the-default-jupyter-application_dashboard[Hiding the default Jupyter application].
endif::[]
ifdef::upstream[]
