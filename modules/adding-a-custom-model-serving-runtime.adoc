...
ifdef::upstream[]
* To learn how to configure a model server that uses a custom model-serving runtime that you have added, see link:{odhdocshome}/working-on-data-science-projects/#configuring-a-model-server-for-your-data-science-project_nb-server[Configuring a model server for your data science project].
endif::[]
ifndef::upstream[]
