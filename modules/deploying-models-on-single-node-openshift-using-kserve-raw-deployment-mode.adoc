...
. By default, OpenShift uses a service mesh for network traffic management. As KServe raw deployment mode does not require a service mesh, disable {org-name} OpenShift Service Mesh:
.. Enter the following command to disable {org-name} OpenShift Service Mesh:
+
[source]
----
...
.. In the Red Hat OpenShift web console *Administrator* view, click *Operators* → *Installed Operators* and then click the {productname-long} Operator.
