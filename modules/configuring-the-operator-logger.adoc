...
. Click *Ecosystem* → *Installed Operators* and then click the {productname-long} Operator.
...
----
for pod in $(oc get pods -l name=rhods-operator -n {operator-default-namespace} -o name); do
  oc logs -f "$pod" -n {operator-default-namespace} &
done
----
+
The Operator pod logs open in your terminal.
...
You can also view each Operator pod log in the {openshift-platform} console by navigating to *Workloads* -> *Pods*, selecting the *`pass:attributes[{operator-default-namespace}]`* project, clicking a pod name, and then clicking the *Logs* tab.
endif::[]