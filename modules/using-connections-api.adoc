...
NOTE: While the Operator does minimal validation of `s3` and `uri` secrets, the overall correctness of the connection secret is the responsibility of the user. We may add more robust validation in future releases.
...
* Without access to a connection Secret, the using resource (workbench, model serving, etc.) fails to start or function.
