...
You can use the Orthogonal Subspace Fine-Tuning (OSFT), Supervised Fine-Tuning (SFT), and Low-Rank Adaption (LoRA) algorithms in the Training Hub.
...
Use LoRA for parameter-efficient fine-tuning with significantly reduced memory requirements by training low-rank adaptation matrices instead of the full model weights. Training Hub implements LoRA with supervised fine-tuning by using the optimized Unsloth backend.
...
Here is a performance comparison of using OSFT, SFT, and LoRA in the Training Hub.
...
+
A rough comparison is: OSFT Memory ~ 3_r_ times SFT Memory, where _r_ is the URR unfreeze rank ratio, the fraction of the matrix being fine-tuned. At URR = 1/3, OSFT and SFT have similar memory usage.
+
...
+
You should keep the `lora_r` value as low as possible. As `lora_r` approaches 0, the memory that LoRA uses should approach ¼ * SFT. While it is difficult to precisely compare SFT and LoRA, LoRA's memory usage should begin to reach or exceed that of SFT's if the value of `lora_r` is more than ⅜ of the size of the hidden dimensionality. Note that the memory used by LoRA in Training Hub is further reduced by the fact that LoRA uses Float16 as its main datatype. QLoRA uses Float4 instead. However, you must briefly place the Float16 model onto the GPU, which can bottleneck memory usage.
