...
Before data scientists in your organization can work with the model registry and model catalog features, you must ensure that the `modelregistry` component is enabled in {productname-short}.
...
ifndef::upstream[]
.. Confirm that the `rhoai-model-registries` namespace is displayed in the *Projects* drop-down list.
endif::[]
