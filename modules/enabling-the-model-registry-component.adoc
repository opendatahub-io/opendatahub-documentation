...
Before data scientists in your organization can work with the model registry and model catalog, you must ensure that the `modelregistry` component is enabled in {productname-short}.

[NOTE]
====
The `modelregistry` component is enabled by default in a new {productname-short} {vernum} installation. However, if the model registry was not enabled in a previous version of {productname-short}, you must enable this component after upgrading to {productname-short} {vernum}.
====
...
ifndef::upstream[]
. Find the `spec.components` section, and then add or update it to include the following `modelregistry` component entry, with the `managementState` field set to `Managed`, and the `registriesNamespace` field set to `pass:attributes[{mr-default-namespace}]`:
+
...
ifdef::upstream[]
.. Confirm that the `odh-model-registries` namespace is displayed in the *Projects* drop-down list.
endif::[]
ifndef::upstream[]
.. Confirm that the `pass:attributes[{mr-default-namespace}]` namespace is displayed in the *Projects* drop-down list.
endif::[]
