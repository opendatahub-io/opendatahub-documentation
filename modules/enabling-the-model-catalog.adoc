...
[role='_abstract']
To allow the data scientists in your organization to register, deploy, and customize models from the {productname-long} Catalog, you must enable the model catalog feature in {productname-short}. You can enable the model catalog feature by setting the `disableModelCatalog` dashboard configuration option to `false`. 
...
[IMPORTANT]
====
The {productname-long} Catalog is not currently supported for disconnected environments.
====
...
ifndef::upstream[]
. Follow the steps described llink:{rhoaidocshome}{default-format-url}/managing_openshift_ai/customizing-the-dashboard[Customizing the dashboard] to set the `disableModelCatalog` dashboard configuration option to `false`. 
endif::[]
