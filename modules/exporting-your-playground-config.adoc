:_module-type: PROCEDURE

[id="exporting-your-playground-configuration_{context}"]
= Exporting your playground configuration

[role="_abstract"]

Export your playgound configuration as a Python code template so that you can use it in your local development environemnt, such as a notebook or IDE.

[NOTE]
====
This code is a template and is not a runnable script. It provides a starting point that shows your configuration, including the model, MCP tools, and RAG files used.
====
 
.Prerequisites

* You have configured your playground with the settings that you want to capture in your code template. This includes:
** Selecting a model.
** Setting model parameters (such as *Temperature*) to your desired values.
** (Optional) Uploading files and enabling the *RAG* function.
** (Optional) Authorizing and enabling any *MCP servers* you intend to use.

.Procedure

. In the playground, configure your desired settings.
. Click the *View code* button.
+
A dialog opens, displaying a Python code template.
. Click *Copy code*.
. Paste the code into your local development environment.
. Install dependencies and connect to the required model and service endpoints to make the code functional.


.Verification

* Review the pasted code in your local environment.
* Confirm that the template includes the correct model, MCP tools, and RAG files from your playground configuration.
