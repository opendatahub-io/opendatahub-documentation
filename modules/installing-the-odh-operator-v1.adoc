...
.Procedure
. Log in to the OpenShift Container Platform as a user with `cluster-admin` privileges. For a developer installation from link:http://try.openshift.com[try.openshift.com,window=blank], you can log in as the `kubeadmin` user.
