...
[id='using-the-cluster-CA-bundle-for-model-serving_{context}']
= Using the cluster-wide CA bundle for the model serving platform

By default, the model serving platform in {productname-short} uses a self-signed certificate generated at installation for the endpoints that are created when deploying a server. 
...
ifndef::upstream[]
* You have configured the model serving platform, as described in link:{rhoaidocshome}{default-format-url}/installing_and_uninstalling_{url-productname-short}/installing-the-model-serving-platform_component-install[Installing the model serving platform].
endif::[]
...
. Navigate to *Ecosystem* -> *Installed Operators* -> *{productname-long}*. 
