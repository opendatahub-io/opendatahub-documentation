...
* _Absolute path_: An absolute path to a specific model file such as `/mnt/models/model.json`
* _Directory path_: A directory path such as `/mnt/models`. If you use a directory path, your model file must use one of the following well-known filenames:
** _XGBoost_: `model.bst`, `model.json`, `model.ubj`
** _LightGBM_: `model.bst`
** _Scikit-learn_: `model.joblib`, `model.pickle`, `model.pkl`
====
+
