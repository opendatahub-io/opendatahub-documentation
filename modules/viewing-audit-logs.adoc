...
ifdef::self-managed[]
NOTE: In {rosa-productname}, audit logging is disabled by default because the Elasticsearch log store does not provide secure storage for audit logs. 
To configure log forwarding, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws/{rosa-latest-version}/html/logging/index[Logging] in the {rosa-productname} documentation.
endif::[]
...
jq 'select((.objectRef.apiGroup == "dscinitialization.opendatahub.io"
              ...
              and .user.username != "system:serviceaccount:{operator-default-namespace}:redhat-ods-operator-controller-manager"
