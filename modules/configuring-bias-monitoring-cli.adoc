...
ifdef::upstream[]
* You are familiar with link:{odhdocshome}{default-format-url}/monitoring-data-science-models/monitoring-models-for-bias#supported-bias-metrics_bias-monitoring[the bias metrics that {productname-short} supports] and how to interpret them.
endif::[]
...
. Configure bias metrics by running a command that uses the following syntax:
+
Syntax:
+
----
...
The request should return output similar to the following:
