...
. Search for the pod that corresponds to the LoRA PyTorch job, that is, *_<training-job-name>_-master-0*.
+
For example, if the training job name is `lora-demo`, the pod name is *lora-demo-master-0*.
