...
. Follow the steps to deploy a model as described in link:{rhoaidocshome}{default-format-url}/deploying_models/rhoai-admin_rhoai-admin#deploying-models-on-the-single-model-serving-platform_rhoai-admin[Deploying models on the single-model serving platform].
