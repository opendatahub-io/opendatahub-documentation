...
The {openshift-platform} cluster that hosts the user's workbench might not have access to enough resources, or the Jupyter pod may have failed.
endif::[]
ifdef::cloud-service[]
The OpenShift cluster that hosts the user's workbench might not have access to enough resources, or the Jupyter pod may have failed.
endif::[]
