...
* `:443/v1/embeddings`
+
NOTE: While the vLLM runtime is compatible with the OpenAI REST API, it is also additionally compatible with the TGIS gRPC API. To query the endpoints for the vLLM runtime with the TGIS gRPC API, refer to the endpoints that are used for the TGIS Standalone runtime. You must also download the files in the `proto` directory of the IBM link:https://github.com/IBM/text-generation-inference[text-generation-inference^] repository, if querying with the TGIS gRPC API.
