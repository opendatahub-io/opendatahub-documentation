...
+
* To use the predefined queue names (`default`), run the following command. Replace `<operator-namespace>` with your operator namespace (the default is `pass:attributes[{operator-default-namespace}]`).  
+
[source,terminal]
----
...
----
+
* To specify custom queue names, run the following command. Replace `<example-cluster-queue>` and `<example-local-queue>` with your custom queue names, and replace `<operator-namespace>` with your operator namespace (the default is `pass:attributes[{operator-default-namespace}]`).  
+
[source,terminal]
----
