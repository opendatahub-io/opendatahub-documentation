...
[NOTE]
====
For metrics to be available for graph generation, you must also enable metrics collection for your deployment. Please see link:{rhoaidocshome}{default-format-url}/managing_and_monitoring_models/cluster-admin_cluster-admin#enabling_metrics_collection_for_an_existing_nim_deployment[Enabling metrics collection for an existing NIM deployment].
====
