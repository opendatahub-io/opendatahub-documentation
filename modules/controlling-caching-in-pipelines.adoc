...
[id='controlling-caching-in-pipelines_{context}']
= Controlling caching in pipelines
...
. On the *Pipeline definitions* page, from the *Project* drop-down list, select the project that contains the pipeline server that you want to configure.
...
ifdef::upstream[]
For more information, see link:{odhdocshome}/working-with-ai-pipelines/#disabling-node-caching-in-elyra_ai-pipelines[Disabling node caching in Elyra].
endif::[]
ifndef::upstream[]
For more information, see link:{rhoaidocshome}{default-format-url}/working_with_ai_pipelines/working-with-pipelines-in-jupyterlab_ai-pipelines#disabling-node-caching-in-elyra_ai-pipelines[Disabling node caching in Elyra].
endif::[]
====
