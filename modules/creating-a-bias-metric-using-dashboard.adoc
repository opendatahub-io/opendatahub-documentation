...
.Prerequisites
ifndef::upstream[]
* You are familiar with xref:supported-bias-metrics[the bias metrics that {productname-short} supports] and how to interpret them.
endif::[]
ifdef::upstream[]
...
+ 
TIP: You can use a `curl` command to query the metadata endpoint and view input attribute names and values. For example: `curl $TRUSTY_ROUTE/info | jq ".[0].data.inputSchema"`
...
+ 
TIP: You can use a `curl` command to query the metadata endpoint and view output attribute names and values. For example: `curl $TRUSTY_ROUTE/info | jq ".[0].data.outputSchema"`
...
.Next step
To view metrics, on the *Bias metric configuration* page, click *View metrics* in the upper-right corner. For more information, see xref:viewing-bias-metrics[Viewing bias metrics for a model].
