...
def ray_integration(openshift_server: str, openshift_token: str): <13>
...
<2> Authenticates with the cluster by using values that you specify when creating the pipeline run; you can omit this section if the Ray cluster is configured to use the same namespace as the data science project
...
<5> Specifies the `LocalQueue` to which the Ray cluster will be submitted: you can omit this line if you configured a default `LocalQueue`
