...
def ray_fn():
   ...
   cluster = Cluster( <3>
       ClusterConfiguration(
           ...
           min_memory=1,
           max_memory=1,
           worker_extended_resource_requests={“nvidia.com/gpu”: 0}, <5>
...
<5> To specify the requested accelerators for the Ray cluster, use the `worker_extended_resource_requests` parameter instead of the deprecated `num_gpus` parameter.
For more details, see the CodeFlare SDK documentation.
