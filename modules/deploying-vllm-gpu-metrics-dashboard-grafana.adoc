...
.. Update the following YAML file, replacing the `$NAMESPACE` and `${MODEL_NAME)` variables with the values of the exported environment variables, and `dashboard_template` with the name of the `GrafanaDashboard` object YAML file that you created earlier:
+
[source]
----
