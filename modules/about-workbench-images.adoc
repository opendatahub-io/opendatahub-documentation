...
To provide a consistent, stable platform for your model development, all provided workbench images contain the same version of Python. Workbench images available on OpenShift AI are pre-built and ready for you to use immediately after OpenShift AI is installed or upgraded. 

Click on view packages to see the packages and versions included in each image.  For information about Red Hat support of notebook images and packages, see Red Hat OpenShift AI: Supported Configurations.
Table 1.1 lists the images that are available by default when you install Red Hat OpenShift AI. 
Important
