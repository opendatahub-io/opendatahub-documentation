...
[role="_abstract"]
A workbench image is preinstalled with tools and libraries for model development. You can use the provided images, or an {productname-short} administrator can create custom images tailored to your needs. 

ifndef::upstream[]
[NOTE]
====
As of {productname-long} version 2.25, the LLM compressor library is generally available and integrated into supported {productname-long} workbench images and AI pipelines.  
...
ifndef::upstream[]
For information about Red Hat support of workbench images and packages, see link:https://access.redhat.com/articles/rhoai-supported-configs-3.x[Supported Configurations for 3.x].
endif::[]
