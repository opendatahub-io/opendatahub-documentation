...
ifdef::upstream[]
The single-model serving platform includes metrics for link:{odhdocshome}/serving-models/#about-the-single-model-serving-platform_serving-large-models[supported runtimes]. You can also configure monitoring for OpenShift Service Mesh. The service mesh metrics helps you to understand dependencies and traffic flow between components in the mesh. When you have configured monitoring, you can grant Prometheus access to scrape the available metrics. 
endif::[]
