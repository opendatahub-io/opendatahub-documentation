...
[subs=+quotes]
....
ENABLE_PGVECTOR=true PGVECTOR_DB=pgvector PGVECTOR_USER=user PGVECTOR_PASSWORD=yourpassword uv run llama stack run run.yaml
....
+
[NOTE]
====
Replace `user` and `yourpassword` with your actual PostgreSQL credentials.
====
...
ENABLE_PGVECTOR=true PGVECTOR_DB=llama-stack PGVECTOR_USER=user PGVECTOR_PASSWORD=yourpassword uv run python beir_benchmarks.py \
  --vector-db-provider-id pgvector
....
