...
[NOTE]
====
The default supported embedding models are `granite-embedding-30m` and `granite-embedding-125m`, served by the `sentence-transformers` framework. Ollama is not required for basic benchmarks but can be used to serve custom embedding models. Before you register additional embedding models, ensure that the Llama Stack server is running and that a vector database provider is enabled.

. Start the Llama Stack server with a vector database provider enabled.
+
--
a. Using remote Milvus:
+
[subs=+quotes]
....
ENABLE_MILVUS=milvus uv run llama stack run run.yaml
....

b. Using remote PostgreSQL with pgvector:
+
[subs=+quotes]
....
ENABLE_PGVECTOR=true uv run llama stack run run.yaml
....
--

. Register the embedding model by using the Llama Stack client. For example:
+
[subs=+quotes]
....
...
* Enter the following command to use the configuration from `run.yaml` and the default dataset `scifact` with remote Milvus:
+
[subs=+quotes]
....
...
[subs=+quotes]
....
ENABLE_PGVECTOR=true uv run python beir_benchmarks.py \
  --vector-db-provider-id remote-pgvector
....
...
[subs=+quotes]
....
LLAMA_STACK_URL="http://localhost:8321" ENABLE_MILVUS=milvus uv run python beir_benchmarks.py
....
...
* *Type:* String.
* *Default:* `milvus`.
* *Example values:* `remote-milvus`, `remote-pgvector`.
* *Example:*
+
[subs=+quotes]
....
