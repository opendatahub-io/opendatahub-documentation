...
[subs=+quotes]
....
PGVECTOR_DB=pgvector uv run llama stack run run.yaml
....
--
...
[subs=+quotes]
....
PGVECTOR_DB=pgvector uv run python beir_benchmarks.py \
...
ENABLE_PGVECTOR=pgvector PGVECTOR_DB=llama-stack PGVECTOR_USER=user PGVECTOR_PASSWORD=yourpassword uv run python beir_benchmarks.py \
