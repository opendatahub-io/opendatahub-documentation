...
* **Llama Stack Operator:** Llama Stack is implemented in {productname-short} through the Llama Stack Operator.
* **The `run.yaml` file:** The `run.yaml` is used to define the APIs and API provider configurations for your server environment. Red Hat ships a default `run.yaml` that includes various APIs and providers with default configurations for simple deployments. You can create a `run.yaml` file to configure advanced workflows and develop custom applications.
...
* **Enabling agentic workflows** using Llama Stack supported APIs, such as Responses and Chat completions.
