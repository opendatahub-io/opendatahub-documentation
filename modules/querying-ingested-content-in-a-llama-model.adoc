...
----
[NOTE]
Ensure your `LlamaStackDistribution` enables the pgvector provider by setting `ENABLE_PGVECTOR=pgvector` and supplies valid PostgreSQL connection details.pgvector is suitable for production-grade RAG workloads that require durable storage, concurrency, and centralized metadata management.
====
