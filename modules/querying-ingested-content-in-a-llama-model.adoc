...
[source,python]
----
vector_store_name = "my_pgvector_db"
vector_store = client.vector_stores.create(
    name=vector_store_name,
    extra_body={
        "embedding_model": embedding_model_id,
        "embedding_dimension": embedding_dimension,
        "provider_id": "pgvector",  # remote PostgreSQL with pgvector
    },
)
vector_store_id = vector_store.id
print(f"Registered pgvector DB: {vector_store_id}")
----
[NOTE]
Ensure that your `LlamaStackDistribution` is configured with a PostgreSQL instance that has the `pgvector` extension enabled and that the pgvector provider is activated (for example, by setting `ENABLE_PGVECTOR=true`).  
pgvector is suitable for production-grade RAG workloads that require durable storage, concurrency, and centralized metadata management.
====
