...
* Aside from the `provider_id`, querying APIs are identical for inline and remote Milvus.
====
====
...
. In a new notebook cell, query the ingested content by using the OpenAI-compatible Responses API with a vector store:
+
[source,python]
----
# Example RAG query for one-off lookups using the Responses API and vector stores
...
print("Low-level query result:", response.output_text)
----
...
* The notebook prints query results for both the low-level Responses API call and the high-level Agent API.
