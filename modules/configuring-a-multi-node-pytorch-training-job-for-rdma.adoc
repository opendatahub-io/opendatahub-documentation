        ...
        spec:
          ...
          - command:
            ...
            - name: NCCL_IB_HCA
              value: "mlx5_1"
----
.. Replace the example container command and example environment-variable values with the appropriate values for your configuration:

... Replace `"your container command"` with the command to be run by your training job.
...
. Specify the details of the nodes that will run the training job, as follows:

.. Copy all of the code that you added for the `Master` node:
+
.Example code for the Master node
[source,subs="+quotes"]
----
    Master:
      replicas: 1
...
.. Paste the copied code at the end of the YAML file.

.. Edit the pasted text to replace `Master` with `Worker`.

.. Edit the Worker `replicas` value to specify how many nodes should be used to run the training job.
