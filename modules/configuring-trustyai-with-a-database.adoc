...
TrustyAI needs to be provided a database configuration secret before deployment. You can create a secret or specify the name of an existing Kubernetes secret within your project. 
...
<3> The password that TrustyAi should use when connecting to the database.
<4> `databaseService` (default, pre-filled value=mariadb) is the Kubernetes (K8s) service that TrustyAI should use when connecting to the database.
<5> The `databasePort` is given a default, pre-filled value=3306 and is the port that TrustyAI should use when connecting to the database.
