...
[id='revoking-user-access-to-workbenches_{context}']
= Revoking user access to workbenches

[role='_abstract']
You can revoke a user's access to workbenches by removing the user from the specialized user groups that define access to {productname-short}. When you remove a user from the specialized user groups, the user is prevented from accessing the {productname-short} dashboard and from using associated services that consume resources in your cluster.
