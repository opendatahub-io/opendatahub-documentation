...
You can deploy generative AI (gen AI) or Predictive AI models on the single-model serving platform by using the *Deploy a model* wizard. The wizard allows you to configure your model, including specifying its location and type, selecting a serving runtime, assigning a hardware profile, and setting advanced configurations like external routes and token authentication.
...
[NOTE]
====
You must add your model as an AI asset endpoint to test your model in the gen AI playground.
====
