...
When you have enabled the single-model serving platform, you can enable a pre-installed or custom model-serving runtime and deploy models on the platform.
...
ifdef::upstream[]
* (Standard deployments only) You have set up your cluster as described in link:{odhdocshome}/serving-models/#deploying-models-on-single-node-openshift-using-kserve-raw-deployment-mode_serving-large-models[Preparing your cluster for standard mode deployment^]
endif::[]
ifndef::upstream[]
