...
* You have NFS, ODF or an equivalent RWX-compatible file system.
...
. From the namespace where you would like to deploy the model, create a PVC for model storage. Create a storage class using `Filesystem volumeMode` and the name of your storage class. The storage size must be larger than the size of the model files on disk. For example:
+
