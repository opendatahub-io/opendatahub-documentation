...
.Procedure
. Create the `vllm-multinode-runtime` ServingRuntime template as follows. The `vllm-multinode-runtime` ServingRuntime uses the same vLLM image as the `vllm-runtime` but it includes information necessary for multi-GPU inferencing.
