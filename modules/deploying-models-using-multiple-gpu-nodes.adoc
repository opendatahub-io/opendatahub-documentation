...
ifndef::upstream[]
* You have enabled the operators for your GPU type, such as Node Feature Discovery Operator, NVIDIA GPU Operator, and so on. For more information about enabling accelerators, see link:{rhoaidocshome}{default-format-url}/managing_openshift_ai/enabling_accelerators[Enabling accelerators^].
endif::[]
ifdef::upstream[]
* You have enabled the operators for your GPU type, such as Node Feature Discovery Operator, NVIDIA GPU Operator, and so on. For more information about enabling accelerators, see link:{odhdocshome}/enabling_accelerators[Enabling accelerators^].
endif::[]

    * You are using one of the following GPU types: `nvidia.com/gpu` (default), `intel.com/gpu`, `amd.com/gpu`, `habana.ai/gaudi`, or you have specified another GPU type in the annotations of the ISVC. 
...
* You have a Persistent Volume Claim (PVC) set up and configured for ReadWriteMany (RWM) access mode. 
...
----
+
. From the namespace where you would like to deploy the model, define a ServingRuntime object in a YAML file called `vllm-multinode-runtime.yaml` with the following contents:
+
[source]
----
... 
...
----
+
. Download the model by exporting your token as an environment variable. In this example, `HF_TOKEN` is the exported token:
+
[source]
----
export HF_TOKEN=__<token>__
...
workerPodName=$(kubectl get pod -l app=isvc.vllm-llama3-8b-predictor-worker --no-headers|cut -d' ' -f1)
