...
This procedure shows you how to serve models on {productname-long} across multiple GPU nodes using the vLLM serving framework. Multi-node inferencing uses the `vllm-multinode-runtime` custom runtime. The `vllm-multinode-runtime` runtime uses the same vLLM image as the `vllm-runtime` runtime and also includes information necessary for multi-GPU inferencing.
