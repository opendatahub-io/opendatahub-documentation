...
* You have read the link:https://www.ibm.com/docs/en/rhocp-ibm-z?topic=spyre-operator-z-linuxone-users-guide[Spyre Operator for IBM Z] documentation.
...
* To use prebuilt model caches for {productname-short} Inference Server 3.4 EA1 images, ensure that you add the following runtime arguments in the `args` section:
** `--max_model_len=32768`
** `--max-num-seqs=32`
** `--tensor-parallel-size=4`
** `--enable-prefix-caching`
* For IBM Z (s390x), set the `VLLM_DT_CHUNK_LEN` environment variable to `512`.
