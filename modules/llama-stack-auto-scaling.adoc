= Enabling high availability on Llama Stack (Optional)

Llama Stack servers can be configured to remain operational in the event of a single point of failure. If a pod restarts, an application crashes, or node maintenance occurs, you can maintain availability by enabling PostgreSQL high-availability settings in your Llama Stack server. The following documentation displays how to enable autoscaling in your `LlamaStackDistribution` custom resource.  
...
. To enable autoscaling to your Llama Stack server, add the following parameters to your `LlamaStackDistribution` CR:
+
[source,yaml]
----
    autoscaling:
...
----
+
.Example `LlamaStackDistribution` CR that enables autoscaling
[source,yaml]
----
