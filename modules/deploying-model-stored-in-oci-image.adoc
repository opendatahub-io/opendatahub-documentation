...
ifndef::upstream[]
* You have stored a model in an OCI image as described in link:{rhoaidocshome}{default-format-url}/serving_models/serving-large-models_serving-large-models#deploying-model-stored-in-oci-image_serving-large-models[Storing a model in an OCI image].
endif::[]
ifdef::upstream[]
