...
* You have installed the `LeaderWorkerSet` Operator in {openshift-platform}. For more information, see the link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/ai_workloads/leader-worker-set-operator[Leader Worker Set Operator documentation].
* If you are running {openshift-platform} on a bare metal cluster: You have set up the MetalLB Operator to provision an external IP address for the `llm-d` Gateway service with the type `LoadBalancer`. For more infomation, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html/ingress_and_load_balancing/load-balancing-with-metallb(Load balancing with MetalLB). Ensure that the Operator is configured as follows:
** Has a standard Kubernetes Service manifest.
** In the `spec` section, has `type:LoadBalancer`. 
