...
[id="configuring-model-control-protocol-servers_{context}"]
...
. Create a file named `mcp-config.yaml` with the following YAML content. You can add multiple server entries under the `data:` field.
+
[source]
----
kind: ConfigMap
...
[IMPORTANT]
====
The `ConfigMap` key (`GitHub-MCP-Server`) must be unique and is case-sensitive. The content provided under this key must be valid JSON format.
====
