...
* Select *Always use the latest pipeline version* to automatically use the most recent pipeline version for each recurring run.
* Select *Use fixed version* to use a specific pipeline version for all recurring runs, and then select the pipeline version to use. Alternatively, to upload a new version, click *Upload new version*, and then complete the relevant fields in the *Upload new version* dialog.
