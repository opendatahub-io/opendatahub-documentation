...
* If you plan to create a `PipelineVersion` custom resource, you have compiled your Python pipeline into YAML by using the KFP SDK. For more information, see _Generating the pipeline YAML with the Kubeflow Pipelines SDK_.
...
[NOTE]
====
Before creating the `PipelineVersion` resource, compile your Python pipeline by using the KFP SDK to produce the YAML required for the `pipelineSpec` field. For more information, see _Generating the pipeline YAML with the Kubeflow Pipelines SDK_.
====
+
