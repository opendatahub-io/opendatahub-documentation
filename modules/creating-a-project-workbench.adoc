...
** `ReadOnlyMany (ROX)`: The storage can be attached to many workbenches in read-only mode.
** `ReadWriteOncePod (RWOP)`: The storage can be attached to a single pod on a single node in read-write mode.
