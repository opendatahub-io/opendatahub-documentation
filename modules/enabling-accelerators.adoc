...
//* **IBM Spyre**: See link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/{ocp-latest-version}/html/managing_openshift_ai/enabling-accelerators#enabling-ibm-spyre_managing-rhoai[Enabling IBM Spyre accelerators]
endif::[]
ifdef::upstream[]
...
* The accelerator is correctly detected a few minutes after full installation of the Node Feature Discovery (NFD) and the relevant accelerator Operator. The {openshift-cli} displays the appropriate output for the GPU worker nodes with accelerator cards. For example, here is output confirming that an NVIDIA GPU is detected: 
+
[source]
----
