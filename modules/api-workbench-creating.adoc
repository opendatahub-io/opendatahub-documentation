...
ifndef::upstream[]
* You know the URL for the image that you want to use in the workbench. The example in this procedure uses the custom image that you created in link:{rhoaidocshome}{default-format-url}/api_workbench/api-custom-image-creating_api-workbench[Creating a custom image by using the `ImageStream` CRD]. 
endif::[]
...
+
<1> The `inject-oauth` annotation generates other OAUTH-based configurations, such as, the `oauth-proxy`, automatically. The default is `true`.
...
<4> The workbench name is visible in the {productname-short} dashboard. In this example, the display name is `my-workbench`.
...
<7> The deployment size for the container. You can set values for the CPU and memory limits and requests.
